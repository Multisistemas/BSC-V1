...
//VERIFICAMOS EL PROCESO
		...
		mysqli_query($link, "INSERT INTO documento_compra (idproveedor,fecha,idempresa)VALUES('$idper','$fecha','$idempresa')");
		$registro = mysqli_query($link, "SELECT max(iddocumento) as maxi FROM documento_compra");
		...
		mysqli_query($link, "insert into detalle_documentocompra(iddocumento,idproducto,cantidad,precio) values('$iddoc','$idpro','$can','$precio')");
		mysqli_query($link, "insert into kardex(fecha,idtipod,idtipoo,serie,nrodocumento,idproducto,cantidad,precio,idusuario) values('$fecha','$tip','C','$serie','$nrodoc','$idpro','$can','$precio','$iduser')");
		mysqli_query($link, "update producto set stock=stock+'$can' where idproducto='$idpro'");
		}
		mysqli_query($link, "update documento_compra set total='$total' where iddocumento='$iddoc'");
		mysqli_query($link, "delete from temporal where idusuario='$iduser'");

//ACTUALIZAMOS LOS REGISTROS Y LOS OBTENEMOS

$registro = mysqli_query($link, "select t.idproducto,p.producto,t.cantidad,p.preciocompra from temporal t,producto p where t.idproducto=p.idproducto and t.idusuario='$iduser'");
