...
//VERIFICAMOS EL PROCESO
		...
		mysql_query("INSERT INTO documento_compra (idproveedor,fecha,idempresa)VALUES('$idper','$fecha','$idempresa')");
		$registro = mysql_query("SELECT max(iddocumento) as maxi FROM documento_compra");
		...
		mysql_query("insert into detalle_documentocompra(iddocumento,idproducto,cantidad,precio) values('$iddoc','$idpro','$can','$precio')");
		mysql_query("insert into kardex(fecha,idtipod,idtipoo,serie,nrodocumento,idproducto,cantidad,precio,idusuario) values('$fecha','$tip','C','$serie','$nrodoc','$idpro','$can','$precio','$iduser')");
		mysql_query("update producto set stock=stock+'$can' where idproducto='$idpro'");
		}
		mysql_query("update documento_compra set total='$total' where iddocumento='$iddoc'");
		mysql_query("delete from temporal where idusuario='$iduser'");

//ACTUALIZAMOS LOS REGISTROS Y LOS OBTENEMOS

$registro = mysql_query("select t.idproducto,p.producto,t.cantidad,p.preciocompra from temporal t,producto p where t.idproducto=p.idproducto and t.idusuario='$iduser'");
