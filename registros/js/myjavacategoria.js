...
$(function(){
	...
	$('#nuevo-categoria').on('click',function(){
		$('#formulario')[0].reset();
		$('#pro').val('Registro');
		$('#edi').hide();
		$('#reg').show();
...
function agregaRegistro(){
	var url = '../categoria/agrega_categoria.php';
	$.ajax({
		type:'POST',
		url:url,
...
function eliminarcategoria(id){
	var url = '../categoria/elimina_categoria.php';
...
function editarcategoria(id){
	$('#formulario')[0].reset();
	var url = '../categoria/edita_categoria.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
