...
//VERIFICAMOS EL PROCESO
		...
		mysqli_query($DB, "INSERT INTO documento_venta (idcliente,fecha,idempresa)VALUES('$idper','$fecha','$idempresa')");
		$registro = mysqli_query($DB, "SELECT max(iddocumento) as maxi FROM documento_venta");
		...
		mysqli_query($DB, "insert into detalle_documentoventa(iddocumento,idproducto,cantidad,precio) values('$iddoc','$idpro','$can','$precio')");
		mysqli_query($DB, "insert into kardex(fecha,idtipod,idtipoo,serie,nrodocumento,idproducto,cantidad,precio,idusuario) values('$fecha','$tip','V','$serie','$nrodoc','$idpro','$can','$precio','$iduser')");
		mysqli_query($DB, "update producto set stock=stock-'$can' where idproducto='$idpro'");
		}
		mysqli_query($DB, "update documento_venta set total='$total' where iddocumento='$iddoc'");
		mysqli_query($DB, "delete from temporal where idusuario='$iduser'");

//ACTUALIZAMOS LOS REGISTROS Y LOS OBTENEMOS

$registro = mysqli_query($DB, "select t.idproducto,p.producto,t.cantidad,p.precioventau from temporal t,producto p where t.idproducto=p.idproducto and t.idusuario='$iduser'");
